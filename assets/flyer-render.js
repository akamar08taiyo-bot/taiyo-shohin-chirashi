...
async function renderFlyer(flyerKey, mountId) {
  const [pagesData, productsData, priceRows] = await Promise.all([
    fetch('./data/pages.json?v=20260902-1').then(r => r.json()),
    fetch('./data/products.json?v=20260902-1').then(r => r.json()),
    fetch('./data/price-rows.json?v=20260902-1').then(r => r.json()),
  ]);
